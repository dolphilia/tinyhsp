    ...
    // GLFWでの描画処理
    {
        ...
        // 文字列を描画
        {
            ...
            for (int i = 0; i < utf8strlen(src_str); i++) {
                ...
                // 空白文字の処理
                // 0x200B // ゼロ幅空白
                if (codepoint_utf16 == 0x20 || // 半角スペース
                    codepoint_utf16 == 0xA0 || // 改行禁止スペース
                    codepoint_utf16 == 0x2002 || // n幅空白
                    codepoint_utf16 == 0x2003 || // m幅空白
                    codepoint_utf16 == 0x2004 || // 1/3
                    codepoint_utf16 == 0x2005 || // 1/4
                    codepoint_utf16 == 0x2009 // 狭い空白
                    )
                {
                    pos_x += (int)((float)font_size * 0.4);
                    continue;
                }
                else if (codepoint_utf16 == 0x3000) // 全角空白
                {
                    pos_x += (int)((float)font_size * 0.8);
                    continue;
                }
