...
main(int argc, char* argv[])
{
    ...
    // 描画処理
    {
        ...
        for (int i = 0; i < line_number_max; i++) {
            ...
            if (strcmp(script[i], "wait") == 0) {
                double time_wait = atof(script[i + 1]);
                i++;
                glfwSetTime(0.0); // タイマーを初期化する
                while (!glfwWindowShouldClose(window)) { // ウィンドウを閉じるまで
                    if (glfwGetTime()*100 > time_wait) {
                        break;
                    }
                    redraw(window, pixel_data, width, height); // 再描画する
